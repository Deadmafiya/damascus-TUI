class Terminal {
    constructor(opts) {
        if (opts.role === "client") {
            ...
            // Damascus UI: load our rcfile (sources ~/.bashrc, then sets short-path PS1).
            // Falls back to eDEX's default params if a custom shell/params are configured.
            let damascusRc = require("path").join(__dirname, "..", "damascus.bashrc");
            let defaultParams = (process.platform === "win32")
                ? []
                : ["--rcfile", damascusRc, "-i"];
            this.tty = this.Pty.spawn(opts.shell || "bash", (opts.params.length > 0 ? opts.params : defaultParams), {
