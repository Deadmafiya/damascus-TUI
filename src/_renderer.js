...
async function initUI() {
    ...
    // Damascus UI: on-screen keyboard removed; filesystem widened (see filesystem.css).
    // Keep a harmless stub so code paths that reference window.keyboard don't throw.
    // Damascus UI: keyboard removed — harmless stub so code paths referencing it don't throw.
    // keydownHandler still plays the typing sound (the on-screen keyboard used to do this).
    // A hidden container with dataset flags is kept because filesystem click handlers read
    // window.keyboard.container.dataset.isCtrlOn / isShiftOn.
    let _kbStubContainer = document.createElement("div");
    _kbStubContainer.style.display = "none";
    _kbStubContainer.dataset.isCtrlOn = false;
    _kbStubContainer.dataset.isShiftOn = false;
    _kbStubContainer.dataset.isAltOn = false;
    _kbStubContainer.dataset.isCapsLckOn = false;
    _kbStubContainer.dataset.isFnOn = false;
    _kbStubContainer.dataset.passwordMode = false;
    document.body.appendChild(_kbStubContainer);
    window.keyboard = {
        linkedToTerm: false,
        container: _kbStubContainer,
        detach: () => {},
        attach: () => {},
        keydownHandler: (e) => {
            try {
                if (e && e.repeat === true &&
                    (e.code || "").match(/^(Shift|Alt|Control|Caps)/)) return;
                window.audioManager.stdin.play();
            } catch (err) { /* audio not ready */ }
        },
        togglePasswordMode: () => {}
    };
    ...
    // Damascus UI: keyboard removed — its reveal animation lines deleted.
    ...
    // Damascus UI: globe replaced with cmatrix placeholder display.
    window.mods.globe = new Cmatrix("mod_column_right");
    ...
    shellContainer.innerHTML += `
        <div id="main_shell_userhost">${require("os").userInfo().username}@${require("os").hostname()}</div>
